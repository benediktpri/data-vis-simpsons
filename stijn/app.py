...
with header_col1:
    ...
    with open("donut.png", "rb") as image_file:
...
# --- Question 1 Chart ---
q1_era_box = alt.Chart(df).mark_boxplot(size=15).encode(
    ...
    color=alt.Color("era:N", scale=era_scale, legend=None), #, legend=alt.Legend(title="Era")
)
q1_era_mean = alt.Chart(df).mark_line(color="black", strokeWidth=2).encode(
    x=alt.X("season:O"), y=alt.Y("mean(imdb_rating):Q")
)
...
# --- Question 2 Chart ---
q2_era_box = alt.Chart(df).mark_boxplot(size=15).encode(
    x=alt.X("season:O", title="Season", axis=alt.Axis(labelAngle=0)),
    y=alt.Y("us_viewers_in_millions:Q", title="US Viewers (millions)"),
    color=alt.Color("era:N", scale=era_scale, legend=None), #, legend=alt.Legend(title="Era")
)
q2_era_mean = alt.Chart(df).mark_line(color="black", strokeWidth=2).encode(
    x=alt.X("season:O"), y=alt.Y("mean(us_viewers_in_millions):Q")
)
...
# --- Question 3 Chart ---
chart3 = alt.Chart(df).mark_circle(size=50, opacity=0.5).encode(
    ...
    tooltip=["title:N", "season:O", "era:N", "imdb_rating:Q", "us_viewers_in_millions:Q"],
).properties(
    height=300, title="Viewers vs Rating"
)
...
chart4 = alt.Chart(df_thu_sun).mark_boxplot(size=40).encode(
    x=alt.X("weekday_label:O", sort=thu_sun_labels, title="Day of Week", axis=alt.Axis(labelAngle=0, labelAlign="center")),
    y=alt.Y("us_viewers_in_millions:Q", title="US Viewers (millions)"),
    color=alt.Color("weekday_label:N", sort=thu_sun_labels, legend=None, scale=alt.Scale(domain=thu_sun_labels, range=["#003f5c"])),
).properties(
    height=300, title="Viewership per day"
)

# --- Question 5 Chart ---
pos_era_abs = df.groupby(["era", "number_in_season"])["us_viewers_in_millions"].agg(["mean", "count"]).reset_index()
pos_era_abs = pos_era_abs[pos_era_abs["count"] >= 3]
era_avgs = df.groupby("era")["us_viewers_in_millions"].mean().reset_index(name="era_mean")
...
q5_lines_points = base.mark_line(strokeWidth=2, point=alt.OverlayMarkDef(filled=True, size=35)).encode(
    tooltip=["era:N", "number_in_season:O", alt.Tooltip("mean:Q", format=".2f"), "count:Q"]
)
q5_refs = alt.Chart(era_avgs).mark_rule(strokeDash=[4, 4], opacity=0.6).encode(
    y="era_mean:Q",
    color=alt.Color("era:N", scale=era_scale, legend=None)
)
